...
class Cf3PromisesFileWriterServiceImpl(
  ...
  private[this] val CSVFILENAME = "reports.csv"
            ...
            cf3PolicyDraftContainer.findById(activeTechniqueId) match {
              ...
              case seq if seq.size == 1 =>
                ...
                val csv = techniqueRepository.getReportingDetailsContent[Seq[String]](technique.id) { optInputStream =>
                    optInputStream match {
                      case None => throw new RuntimeException(s"Error when trying to open reports descriptor for technique ${technique}. Check that the report descriptor exist and is correctly commited in Git, or that the metadata for the technique are corrects.")
                      case Some(inputStream) =>
